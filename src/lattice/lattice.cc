...
int Lattice::construct_graph(void) 
{
  ...
  vertex_index = get(boost::vertex_index, g);
  ...
  for (unsigned i=0; i<bonds.size(); ++i) {
    source = vertex(bonds[i].src_id(), g);
    target = vertex(bonds[i].tgt_id(), g);
...
bool Lattice::connect_bond(Bond& bond) const
{
  ...
  bond.connect(s, src_offset, t, tgt_offset, sign);
  return true;
}
...
int Lattice::mapped_site_id(const unsigned& local_id, const Vector3i& bravindex) const
{
  ...
  return (static_cast<int>(local_id) + cell_id * unitcell.num_sites());
}
